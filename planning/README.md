...
if [ "$ARCH" = "aarch64" ]; then
    ...
    if ip link show eth0 > /dev/null 2>&1; then
        ...
        ip addr add 192.168.0.20/24 dev eth0 || echo "IP 192.168.0.20 may already be assigned to eth0."
        ip link set eth0 up
    else
    ...
    if ip link show eno1 > /dev/null 2>&1; then
        ip addr add 192.168.0.20/24 dev eno1 || echo "IP 192.168.0.20 may already be assigned to eno1."
        ip link set eno1 up
    else
